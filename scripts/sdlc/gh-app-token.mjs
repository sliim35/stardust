...
//
// The private key is read from the environment (agent secrets), in this order:
//   GH_APP_PRIVATE_KEY_B64   base64 of the .pem   (preferred: single-line, secret-store friendly)
//   GH_APP_PRIVATE_KEY       raw PEM string
...
const loadPem = () => {
  ...
  return die('no private key — set GH_APP_PRIVATE_KEY_B64 (preferred), GH_APP_PRIVATE_KEY, or GH_APP_PRIVATE_KEY_PATH')
}

const b64url = (input) =>
  Buffer.from(input).toString('base64').replaceAll('+', '-').replaceAll('/', '_').replace(/=+$/, '')

// Sign a GitHub App JWT (RS256). exp is 8 min out with iat backdated 60s for clock skew,
// keeping exp - iat well under GitHub's 10-minute maximum.
const mintJwt = (pem) => {
