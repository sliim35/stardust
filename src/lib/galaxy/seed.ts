...
export const DEFAULT_BACKDROP: GalaxyBackdrop = {
  seed: 7777,
  branches: 4,
  spin: 1,
  randomnessPower: 2.2,
...
function memHash(str: string): number {
  let h = 2166136261;
...
function memRng(seed: number): () => number {
  let a = seed >>> 0;
  return () => {
    a = (a + 0x6d2b79f5) | 0;
...
export function placeStar(
  ...
  const m = MOODS[mood] ?? MOODS.wistful;
...
export function buildSeedSky(): GalaxySky {
  const stars: MemoryStar[] = SEED.map((s, i) => {
