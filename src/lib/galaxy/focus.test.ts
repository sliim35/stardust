...
describe("cancelFocus — a user drag cancels the in-flight ease", () => {
  it("stops easing (target := current) and is no longer focusing", () => {
    const s0 = createFocus(DEFAULT_FRAMING);
    const s1 = focusCamera(s0, cam(760, 440, 1.8));
    const mid = stepFocus(s1, 0.3); // in-flight
    const cancelled = cancelFocus(mid);
    expect(cancelled.focusing).toBe(false);
    expect(cancelled.target).toEqual(cancelled.current); // rests where the drag took over
  });

  it("keeps prior framing so ESC/back still works after a drag-cancel", () => {
    const s0 = createFocus(DEFAULT_FRAMING);
    const s1 = focusCamera(s0, cam(760, 440, 1.8));
